    ...
    <div class="ds-section">
      ...
      <div class="ds-diag-grid">
        ...
        ${i.length?q`<div class="ds-diag-item ds-diag-alert"><span class="ds-diag-label">Alerts</span><span class="ds-diag-val">${i.join(", ")}</span></div>`:Q}
          ...
          </div>`;const A=s.map(e=>e.v),C=(this._config.graph_style?.sensor_ranges??{})[k]??{},S=C.min??Math.min(...A),E=C.max??Math.max(...A),T=E-S||1,I=s[0].t,z=s[s.length-1].t,M=z-I||1,D=e=>(e.t-I)/M*a,O=e=>n-4-(e.v-S)/T*(n-8),R=s.map(e=>`${D(e).toFixed(1)},${O(e).toFixed(1)}`).join(" "),P=D(s[0]).toFixed(1),$=`sg-${o.replace(/[^a-z0-9]/gi,"")}`,B=A[A.length-1],F=B%1==0?`${B}`:B.toFixed(1),L=Math.max(...A),N=Math.min(...A);let j=A.indexOf(L),H=A.indexOf(N);j<0&&(j=0),H<0&&(H=0);const U=D(s[j]).toFixed(1),W=O(s[j]).toFixed(1),V=D(s[H]).toFixed(1),Z=O(s[H]).toFixed(1),X=y(s[0].t),Y=y((s[0].t+z)/2),K=f/M*a,J=`${Math.max(.3,.7*K).toFixed(2)} ${Math.max(.3,.3*K).toFixed(2)}`,ee=c&&E-S>0;return q`
          ...
          <div class="spark-row ${i?"":"spark-row-clickable"}" @click=${i?Q:t=>{t.stopPropagation(),this._detailDevice=e.device_id}}>
            ...
            <div class="spark-svg-wrap">
              <svg viewBox="0 0 ${a} ${n}" preserveAspectRatio="none"
                ...
                `:Q}
                ${"bar"===h?s.map(e=>{const t=Math.max(2,a/s.length-2),i=D(e)-t/2,o=O(e),r=n-4-o;return G`<rect x="${i.toFixed(1)}" y="${o.toFixed(1)}" width="${t.toFixed(1)}" height="${Math.max(0,r).toFixed(1)}" rx="${u}" fill="${_}" opacity="0.75"/>`}):G`
    ...
    `:Q;return p.length||u||o||a||n||S.length>1?q`
      <div class="ts-lower-body">
        ...
        <div class="tile tile--clickable ${i?"":"offline"} tile-${n}" style=${_e(l)}
          ...
          ${Oe(t,e=>e.join("+"),e=>1===e.length?Vi(a,e[0]):q`<div class="tile-row">${e.map(e=>Vi(a,e))}</div>`)}
      ...
      <div class="area-section ${s?"closed":""} ${a?.bg_image&&"ambient"===a.bg_image_mode?"area-bg-ambient":""}" style=${_e(r)}>
        ...
        ${s?Q:q`
          ...
          <div class="device-grid" style="--cols:${n}">
            ${Oe(t,e=>e.device_id,e=>this._renderTile(e,l))}
          </div>
        `}
      </div>
