    ...
    <div class="ds-section">
      ...
      <div class="ds-diag-grid">
        ...
        ${i.length?q`<div class="ds-diag-item ds-diag-alert"><span class="ds-diag-label">Alerts</span><span class="ds-diag-val">${i.join(", ")}</span></div>`:K}
          ...
          </div>`;const A=s.map(e=>e.v),S=s.filter(e=>!e.live).map(e=>e.v),C=(this._config.graph_style?.sensor_ranges??{})[k]??{},E=C.min??Math.min(...S),T=C.max??Math.max(...S),I=T-E||1,z=s[0].t,O=s[s.length-1].t,D=O-z||1,M=e=>(e.t-z)/D*a,R=e=>r-4-(e.v-E)/I*(r-8),$=s.map(e=>`${M(e).toFixed(1)},${R(e).toFixed(1)}`).join(" "),P=M(s[0]).toFixed(1),B=`sg-${o.replace(/[^a-z0-9]/gi,"")}`,F=A[A.length-1],L=F%1==0?`${F}`:F.toFixed(1),N=Math.max(...S),j=Math.min(...S);let H=A.indexOf(N),W=A.indexOf(j);H<0&&(H=0),W<0&&(W=0);const U=M(s[H]).toFixed(1),V=R(s[H]).toFixed(1),Q=M(s[W]).toFixed(1),X=R(s[W]).toFixed(1),Y=y(s[0].t),Z=y((s[0].t+O)/2),J=f/D*a,ee=`${Math.max(.3,.7*J).toFixed(2)} ${Math.max(.3,.3*J).toFixed(2)}`,te=c&&T-E>0;return q`
          ...
          <div class="spark-row ${i?"":"spark-row-clickable"}" @click=${i?K:t=>{t.stopPropagation(),this._detailDevice=e.device_id}}>
            ...
            <div class="spark-svg-wrap">
              <svg viewBox="0 0 ${a} ${r}" preserveAspectRatio="none"
                ...
                @mousemove=${e=>{const t=e.currentTarget,i=t.getBoundingClientRect(),o=Math.max(0,Math.min(1,(e.clientX-i.left)/i.width)),r=z+o*D;let n=s[0];for(const e of s)Math.abs(e.t-r)<Math.abs(n.t-r)&&(n=e);const l=M(n),c=R(n),d=t.querySelector(".spark-crosshair");d&&(d.setAttribute("x1",String(l)),d.setAttribute("x2",String(l)),d.style.display="");const p=t.querySelector(".spark-hover-dot");p&&(p.setAttribute("cx",String(l)),p.setAttribute("cy",String(c)),p.style.display="");const h=t.parentElement,g=h?.querySelector(".spark-tooltip");if(g){const e=g.querySelector(".spark-tooltip-val"),t=g.querySelector(".spark-tooltip-time");e&&(e.textContent=`${n.v%1==0?String(n.v):n.v.toFixed(1)} ${w}`),t&&(t.textContent=y(n.t)),g.style.left=`${(l/a*100).toFixed(1)}%`,g.style.display=""}}} @mouseleave=${e=>{const t=e.currentTarget;t.querySelector(".spark-crosshair")?.style&&(t.querySelector(".spark-crosshair").style.display="none"),t.querySelector(".spark-hover-dot")?.style&&(t.querySelector(".spark-hover-dot").style.display="none");const i=t.parentElement?.querySelector(".spark-tooltip");i&&(i.style.display="none")}}>
                <defs>
                ...
                `:K}
                ${"bar"===h?s.map(e=>{const t=Math.max(2,a/s.length-2),i=M(e)-t/2,o=R(e),n=r-4-o;return G`<rect x="${i.toFixed(1)}" y="${o.toFixed(1)}" width="${t.toFixed(1)}" height="${Math.max(0,n).toFixed(1)}" rx="${g}" fill="${_}" opacity="0.75"/>`}):G`
    ...
    `:K;return p.length||g||o||a||r||C.length>1?q`
      <div class="ts-lower-body">
        ...
        <div class="tile tile--clickable ${i?"":"offline"} tile-${r}" style=${_e(p)}
          ...
          ${Me(t,e=>e.join("+"),e=>1===e.length?Qi(a,e[0]):q`<div class="tile-row">${e.map(e=>Qi(a,e))}</div>`)}
      ...
      <div class="area-section ${s?"closed":""} ${a?.bg_image&&"ambient"===a.bg_image_mode?"area-bg-ambient":""}" style=${_e(n)}>
        ...
        ${s?K:q`
          ...
          <div class="device-grid" style="--cols:${r}">
            ${Me(t,e=>e.device_id,e=>this._renderTile(e,l))}
          </div>
        `}
      </div>
    `}render(){if(!this._config||!this.hass)return q``;const e=new Set(["hui-card-picker","hui-cards-used-card-picker"]);let t=this,i=!1;for(;t;){if(t instanceof Element&&e.has(t.tagName.toLowerCase())){i=!0;break}const s=t.getRootNode();if(s===t||s===document)break;t=s.host}if(i)return q`
      ...
      <ha-card class=${this._config.effects?"":"no-fx"} style=${_e(u)} @click=${()=>{this._cloudDetailOpen&&(this._cloudDetailOpen=null)}}>
        ...
        <div class="dash-body">
          ...
          ${l?Me([...r.entries()],([e])=>e,([e,t])=>this._renderAreaSection(e,t)):q`<div class="device-grid" style="--cols:${o?.columns??this._config.columns??3}">
                ${Me(a,e=>e.device_id,e=>this._renderTile(e,this._config.area_styles?.[e.area??""]?.tile_style))}
              </div>`}
        </div>
