    ...
    <div class="ds-section">
      ...
      <div class="ds-diag-grid">
        ...
        ${i.length?q`<div class="ds-diag-item ds-diag-alert"><span class="ds-diag-label">Alerts</span><span class="ds-diag-val">${i.join(", ")}</span></div>`:K}
          ...
          </div>`;const A=s.map(e=>e.v),S=(this._config.graph_style?.sensor_ranges??{})[k]??{},C=S.min??Math.min(...A),E=S.max??Math.max(...A),T=E-C||1,I=s[0].t,z=s[s.length-1].t,D=z-I||1,O=e=>(e.t-I)/D*a,M=e=>n-4-(e.v-C)/T*(n-8),$=s.map(e=>`${O(e).toFixed(1)},${M(e).toFixed(1)}`).join(" "),R=O(s[0]).toFixed(1),P=`sg-${o.replace(/[^a-z0-9]/gi,"")}`,B=A[A.length-1],F=B%1==0?`${B}`:B.toFixed(1),L=Math.max(...A),N=Math.min(...A);let j=A.indexOf(L),H=A.indexOf(N);j<0&&(j=0),H<0&&(H=0);const W=O(s[j]).toFixed(1),U=M(s[j]).toFixed(1),V=O(s[H]).toFixed(1),Q=M(s[H]).toFixed(1),X=y(s[0].t),Y=y((s[0].t+z)/2),Z=f/D*a,J=`${Math.max(.3,.7*Z).toFixed(2)} ${Math.max(.3,.3*Z).toFixed(2)}`,ee=c&&E-C>0;return q`
          ...
          <div class="spark-row ${i?"":"spark-row-clickable"}" @click=${i?K:t=>{t.stopPropagation(),this._detailDevice=e.device_id}}>
            ...
            <div class="spark-svg-wrap">
              <svg viewBox="0 0 ${a} ${n}" preserveAspectRatio="none"
                ...
                @mousemove=${e=>{const t=e.currentTarget,i=t.getBoundingClientRect(),o=Math.max(0,Math.min(1,(e.clientX-i.left)/i.width)),n=I+o*D;let r=s[0];for(const e of s)Math.abs(e.t-n)<Math.abs(r.t-n)&&(r=e);const l=O(r),c=M(r),d=t.querySelector(".spark-crosshair");d&&(d.setAttribute("x1",String(l)),d.setAttribute("x2",String(l)),d.style.display="");const p=t.querySelector(".spark-hover-dot");p&&(p.setAttribute("cx",String(l)),p.setAttribute("cy",String(c)),p.style.display="");const h=t.parentElement,g=h?.querySelector(".spark-tooltip");if(g){const e=g.querySelector(".spark-tooltip-val"),t=g.querySelector(".spark-tooltip-time");e&&(e.textContent=`${r.v%1==0?String(r.v):r.v.toFixed(1)} ${w}`),t&&(t.textContent=y(r.t)),g.style.left=`${(l/a*100).toFixed(1)}%`,g.style.display=""}}} @mouseleave=${e=>{const t=e.currentTarget;t.querySelector(".spark-crosshair")?.style&&(t.querySelector(".spark-crosshair").style.display="none"),t.querySelector(".spark-hover-dot")?.style&&(t.querySelector(".spark-hover-dot").style.display="none");const i=t.parentElement?.querySelector(".spark-tooltip");i&&(i.style.display="none")}}>
                <defs>
                ...
                `:K}
                ${"bar"===h?s.map(e=>{const t=Math.max(2,a/s.length-2),i=O(e)-t/2,o=M(e),r=n-4-o;return G`<rect x="${i.toFixed(1)}" y="${o.toFixed(1)}" width="${t.toFixed(1)}" height="${Math.max(0,r).toFixed(1)}" rx="${g}" fill="${_}" opacity="0.75"/>`}):G`
