    ...
    <button
      ...
      <div class="ts-sensor" style="--ts-accent:${i}">
        <div class="ts-sensor-top">
          ${at(t,s)}
          ${f&&w?q`<div class="ts-chips ts-chips-hdr">${w}</div>`:Y}
        ...
        ${m&&w?q`<div class="ts-chips" style="margin-top:6px">
          ${w}
        </div>`:Y}
    ...
    <div class="ds-section">
      ...
      <div class="cf-backdrop" @click=${t}>
        <div class="cf-box" role="alertdialog" aria-modal="true" aria-label=${it("confirm.aria")}
          ...
          </div>`;const $=s.map(e=>e.v),E=s.filter(e=>!e.live).map(e=>e.v),T=(this._config.graph_style?.sensor_ranges??{})[A]??{},I=T.min??Math.min(...E),M=T.max??Math.max(...E),D=M-I||1,O=s[0].t,z=s[s.length-1].t,P=z-O||1,R=e=>(e.t-O)/P*a,B=e=>n-4-(e.v-I)/D*(n-8),L=s.map(e=>`${R(e).toFixed(1)},${B(e).toFixed(1)}`).join(" "),F=R(s[0]).toFixed(1),N=`sg-${o.replace(/[^a-z0-9]/gi,"")}`,j=$[$.length-1],H=j%1==0?`${j}`:j.toFixed(1),U=Math.max(...E),W=Math.min(...E);let V=$.indexOf(U),K=$.indexOf(W);V<0&&(V=0),K<0&&(K=0);const X=R(s[V]).toFixed(1),Q=B(s[V]).toFixed(1),Z=R(s[K]).toFixed(1),J=B(s[K]).toFixed(1),ee=x(s[0].t),te=x((s[0].t+z)/2),ie=m/P*a,se=`${Math.max(.3,.7*ie).toFixed(2)} ${Math.max(.3,.3*ie).toFixed(2)}`,oe=c&&M-I>0,ae=e=>e%1==0?`${e}`:e.toFixed(1),ne=e=>h?q`
        <span class="spark-axis spark-axis-${e}" aria-hidden="true">
          <i>${ae(M)}</i><i>${ae(I)}</i>
        </span>`:Y;return q`
