    ...
    <div class="ds-section">
      ...
      <div class="ds-diag-grid">
        ...
        ${i.length?q`<div class="ds-diag-item ds-diag-alert"><span class="ds-diag-label">Alerts</span><span class="ds-diag-val">${i.join(", ")}</span></div>`:K}
          ...
          </div>`;const A=s.map(e=>e.v),S=(this._config.graph_style?.sensor_ranges??{})[k]??{},C=S.min??Math.min(...A),E=S.max??Math.max(...A),T=E-C||1,I=s[0].t,z=s[s.length-1].t,O=z-I||1,$=e=>(e.t-I)/O*a,D=e=>n-4-(e.v-C)/T*(n-8),M=s.map(e=>`${$(e).toFixed(1)},${D(e).toFixed(1)}`).join(" "),R=$(s[0]).toFixed(1),P=`sg-${o.replace(/[^a-z0-9]/gi,"")}`,B=A[A.length-1],F=B%1==0?`${B}`:B.toFixed(1),L=Math.max(...A),N=Math.min(...A);let j=A.indexOf(L),H=A.indexOf(N);j<0&&(j=0),H<0&&(H=0);const W=$(s[j]).toFixed(1),U=D(s[j]).toFixed(1),V=$(s[H]).toFixed(1),Q=D(s[H]).toFixed(1),X=y(s[0].t),Y=y((s[0].t+z)/2),Z=f/O*a,J=`${Math.max(.3,.7*Z).toFixed(2)} ${Math.max(.3,.3*Z).toFixed(2)}`,ee=c&&E-C>0;return q`
          ...
          <div class="spark-row ${i?"":"spark-row-clickable"}" @click=${i?K:t=>{t.stopPropagation(),this._detailDevice=e.device_id}}>
            ...
            <div class="spark-svg-wrap">
              <svg viewBox="0 0 ${a} ${n}" preserveAspectRatio="none"
                ...
                `:K}
                ${"bar"===h?s.map(e=>{const t=Math.max(2,a/s.length-2),i=$(e)-t/2,o=D(e),r=n-4-o;return G`<rect x="${i.toFixed(1)}" y="${o.toFixed(1)}" width="${t.toFixed(1)}" height="${Math.max(0,r).toFixed(1)}" rx="${g}" fill="${_}" opacity="0.75"/>`}):G`
