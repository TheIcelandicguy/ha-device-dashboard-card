    ...
    <div class="ds-section">
      ...
      <div class="cf-backdrop" @click=${t}>
        <div class="cf-box" role="alertdialog" aria-modal="true" aria-label=${it("confirm.aria")}
          ...
          </div>`;const C=s.map(e=>e.v),$=s.filter(e=>!e.live).map(e=>e.v),E=(this._config.graph_style?.sensor_ranges??{})[k]??{},T=E.min??Math.min(...$),I=E.max??Math.max(...$),M=I-T||1,D=s[0].t,O=s[s.length-1].t,z=O-D||1,P=e=>(e.t-D)/z*a,R=e=>n-4-(e.v-T)/M*(n-8),B=s.map(e=>`${P(e).toFixed(1)},${R(e).toFixed(1)}`).join(" "),L=P(s[0]).toFixed(1),F=`sg-${o.replace(/[^a-z0-9]/gi,"")}`,N=C[C.length-1],j=N%1==0?`${N}`:N.toFixed(1),H=Math.max(...$),U=Math.min(...$);let W=C.indexOf(H),V=C.indexOf(U);W<0&&(W=0),V<0&&(V=0);const K=P(s[W]).toFixed(1),X=R(s[W]).toFixed(1),Q=P(s[V]).toFixed(1),Z=R(s[V]).toFixed(1),J=w(s[0].t),ee=w((s[0].t+O)/2),te=f/z*a,ie=`${Math.max(.3,.7*te).toFixed(2)} ${Math.max(.3,.3*te).toFixed(2)}`,se=c&&I-T>0;return q`
